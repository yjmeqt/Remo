...
remo-cli           — CLI entry point (clap), delegates to remo-desktop
```
...
The `remo-sdk` crate compiles to a C static library for iOS. Five exported C functions: `remo_start`, `remo_stop`, `remo_register_capability`, `remo_free_string`, `remo_list_capabilities`. The C header is generated by cbindgen into `swift/RemoSwift/Sources/RemoSwift/include/remo.h`.
