...
#[must_use = "dropping a ServiceRegistration immediately de-registers the service"]
...
// ctx_ptr is only reclaimed in Drop after the event loop is aborted.
#[allow(unsafe_code)]
...
impl ServiceRegistration {
    ...
    pub fn register(
        ...
        // ctx is an Arc we intentionally leak; reclaimed in Drop.
        let err = unsafe {
    ...
    /// Whether the registration callback has confirmed success.
...
impl Drop for ServiceRegistration {
    ...
    fn drop(&mut self) {
        ...
        if !self.ctx_ptr.is_null() {
            // SAFETY: ctx_ptr was created via Arc::into_raw in register().
            // The callback only peeks + forgets, so refcount is still 1.
            unsafe { Arc::from_raw(self.ctx_ptr as *const AtomicBool) };
        }
    }
}
...
unsafe extern "C" fn register_callback(
    ...
    // the Drop impl on ServiceRegistration reclaims this Arc.
